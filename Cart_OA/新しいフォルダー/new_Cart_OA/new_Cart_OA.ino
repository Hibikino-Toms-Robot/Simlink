...
// ↓ int_data[1]にする理由　2byte送信する方法
// https://hawksnowlog.blogspot.com/2016/11/sending-multibytes-with-serialwrite.html#int-2byte-%E3%81%AE%E6%83%85%E5%A0%B1%E3%82%92%E9%80%81%E4%BF%A1%E3%81%99%E3%82%8B%E6%96%B9%E6%B3%95
int send_data = 0;
...
volatile int cart_direction = 1;     //現在の移動方向(左:1 右:-1 エラー:0)
...
void RecognizeRotation(void){
  if (cart_direction == 1) { pulse++; }
  else if (cart_direction == -1) { pulse--; }
}
...
void Calculate_distance(void){
  ...
  float distance_m = rot * (2.0 * 3.1416 * r);
  send_data = distance_m * 100;
...
void loop() {
  ...
  if(Serial.available() > 0)
  {
    ...
    /*
    if (cart_direction == 1) { send_data++; }
    else if (cart_direction == -1) { send_data--; }

    
    if (send_data >= 300 || send_data <= -300) { send_data = 0; }
    */
